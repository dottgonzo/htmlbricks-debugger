...
export const oauth2providers: Readable<{ provider: string, uri: string }[]> = readable([
	{
		provider: 'google',
		uri: `https://accounts.google.com/o/oauth2/v2/auth?scope=${googleScope}&include_granted_scopes=true&response_type=token&state=state_parameter_passthrough_value&redirect_uri=${developmentFontendUrl}/login&client_id=${googleClientId}`
	},
	{
		provider: 'facebook',
		uri: ''
...
export const componentsVersion: Readable<string> = readable('0.15.10');
